					...
					jQuery
							.extend({
								...
								Deferred : function(func) {
									...
									// deferred[ resolveWith | rejectWith |
									// notifyWith ]
									// promise[ done | fail | progress | then |
									// always | state | promise ]

									// 调用内部辅助的promise的promise方法（jQ坑爹，起同样名字）
									// 扩展deferred的then | done | fail |
									// progress等方法
									promise.promise(deferred);

									// 这里为什么要分成promise跟deferred两个对象呢
									// 其实就是因为不想把[resolve | reject |
									// notify]这几个函数暴露出去
									//
								...
								when : function(subordinate /*
									...
									var i = 0,
									// 任务数量
									resolveValues = slice.call(arguments), length = resolveValues.length,
...
/*
 * ! jQuery Transit - CSS3 transitions and transformations (c) 2011-2014 Rico
 * Sta. Cruz MIT Licensed.
 * 
		...
		(
				...
				function($) {
					...
					$.transit = {
						...
						// Set this to false if you don't want to use the
						// transition end property.
					...
					function getVendorPropertyName(prop) {
						...
						var prop_ = prop.charAt(0).toUpperCase()
								+ prop.substr(1);
						// 增加前缀
					...
					var isChrome = navigator.userAgent.toLowerCase().indexOf(
							'chrome') > -1;
					...
					var transitionEnd = support.transitionEnd = eventNames[support.transition]
							|| null;

					// Populate jQuery's `$.support` with the vendor prefixes we
					// know.
					// As per [jQuery's cssHooks
					// documentation](http://api.jquery.com/jQuery.cssHooks/),
					// we set $.support.transition to a string of the actual
					// property name used.
					for ( var key in support) {
						if (support.hasOwnProperty(key)
								&& typeof $.support[key] === 'undefined') {
					...
					// ## $.cssEase
					// List of easing aliases that you can use with
					// `$.fn.transition`.
					$.cssEase = {
						'_default' : 'ease',
						'in' : 'ease-in',
					...
					$.cssHooks['transit:transform'] = {
						...
						set : function(elem, v) {
							...
							// We've seen the 3D version of Scale() not work in
							// Chrome when the
							// element being scaled extends outside of the
							// viewport. Thus, we're
							// forcing Chrome to not use the 3d transforms as
							// well. Not sure if
							// translate is affectede, but not risking it.
							// Detection code from
					...
					// In jQuery 1.8+, this will intentionally override the
					// default `transform`
					...
					// jQuery 1.8+ supports prefix-free transitions, so these
					// polyfills will not
					// be necessary.
					if ($.fn.jquery < "1.8") {
						// ## 'transformOrigin' CSS hook
						// Allows the use for `transformOrigin` to define where
					...
					// ## Transform class
					// This is the main class of a transformation property that
					// powers
					// `$.fn.css({ transform: '...' })`.
					//
					// This is, in essence, a dictionary object with key/values
					// as `-transform`
					// properties.
					//
					...
					// t.rotate //=> "4deg"
					//
					// Convert it to a CSS string using the `toString()` and
					// `toString(true)` (for WebKit)
					// functions.
					//
					// t.toString() //=> "rotate(90deg) scale(4,4)"
					// t.toString(true) //=> "rotate(90deg) scale3d(4,4,0)"
					// (WebKit version)
					//
					...
					Transform.prototype = {
						...
						setFromString : function(prop, val) {
							var args = (typeof val === 'string') ? val
									.split(',')
						...
						setter : {
							...
							// ### x / y
							// Translations. Notice how this keeps the other
							// value.
							//
							...
							//
							// .css({ translate: '2, 5' }) //=> "translate(2px,
							// 5px)"
							//
						...
						getter : {
							...
							rotate3d : function() {
								var s = (this.rotate3d || "0,0,0,0deg")
										.split(',');
								for (var i = 0; i <= 3; ++i) {
									if (s[i]) {
										s[i] = parseFloat(s[i]);
						...
						parse : function(str) {
							var self = this;
							str.replace(/([a-zA-Z0-9]+)\((.*?)\)/g, function(x,
									prop, val) {
						...
						// ### toString()
						// Converts to a `transition` CSS property string. If
						// `use3d` is given,
						// it converts to a `-webkit-transition` CSS property
						// string instead.
						toString : function(use3d) {
							...
							for ( var i in this) {
								if (this.hasOwnProperty(i)) {
									// Don't use 3D transformations if the
					...
					// ### getProperties(dict)
					// Returns properties (for `transition-property`) for
					// dictionary `props`. The
					// value of `props` is what you would expect in
					// `$.css(...)`.
					function getProperties(props) {
						var re = [];

						$.each(props, function(key) {
							key = $.camelCase(key); // Convert "text-align" =>
													// "textAlign"
							key = $.transit.propertyMap[key] || $.cssProps[key]
									|| key;
					...
					// ### getTransition()
					// Returns the transition string to be used for the
					...
					// });
					//
					$.fn.transition = $.fn.transit = function(properties,
						...
						if (typeof duration === 'object') {
							...
							queue = typeof duration.queue === "undefined" ? true
									: duration.queue;
							callback = duration.complete;
							duration = duration.duration;
						}

						// Account for `.transition(properties, duration,
						// callback)`.
						...
						var transitionValue = getTransition(theseProperties,
								duration, easing, delay);

						// Compute delay until callback.
						// If this becomes 0, don't bother setting the
						// transition property.
						var work = $.transit.enabled && support.transition;
						var i = work ? (parseInt(duration, 10) + parseInt(
								delay, 10)) : 0;
						...
						// Save the old transitions of each element so we can
						// restore it later.
						var oldTransitions = {};
						...
						var run = function(nextCall) {
							...
							if ((i > 0) && (transitionEnd)
								...
								// Use the 'transitionend' event if it's
								// available.
								bound = true;
								self.bind(transitionEnd, cb);
							} else {
								// Fallback to timers if the 'transitionend'
						...
						// Defer running. This allows the browser to paint any
						// pending CSS it hasn't
						// painted yet before doing the transitions.
						var deferredRun = function(next) {
							this.offsetWidth = this.offsetWidth; // force a
																	// repaint
							run(next);
						};
					...
					function registerCssHook(prop, isPixels) {
						// For certain properties, the 'px' should not be
						// implied.
						if (!isPixels) {
							$.cssNumber[prop] = true;
						}
					...
					// ### unit(number, unit)
					// Ensures that number `number` has a unit. If no unit is
					// found, assume the
					// default is `unit`.
					//
					// unit(2, 'px') //=> "2px"
					...
					function toMS(duration) {
						...
						// Allow string durations like 'fast' and 'slow',
						// without overriding numeric values.
	...
	function BoyWalk() {
		...
		return {
			...
			resetOriginal : function() {
				this.stopWalk();
				$boy.removeClass("slowWalk slowFlolerWalk").addClass(
						"boyOriginal")
			},
			toShop : function() {
		...
		setInterval(
				function() {
					...
					endPositionTop = visualHeight - 40,
							endPositionLeft = startPositionLeft - 100
									+ Math.random() * 500,
							duration = visualHeight * 10 + Math.random() * 5000;
					var randomStart = Math.random();
					randomStart = randomStart < 0.5 ? startOpacity
							: randomStart;
